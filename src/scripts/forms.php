...
/*  Copyright 2005-2026 Patrick R. Michaud (pmichaud@pobox.com)
...
foreach(array('text', 'submit', 'hidden', 'password', 'reset', 'file',
    ...
    'color', 'range', 'button') as $t) 
  SDV($InputTags[$t][':html'], "<input type='$t' \$InputFormArgs />");

foreach(array('text', 'textarea', 'password', 'email', 'url', 'tel', 'number', 'search', 'date', 'month') as $t) {
  SDV($InputTags[$t]['class'], "inputbox");
  SDV($InputTags[$t][':args'], array('name', 'value', 'placeholder'));
}
...
function InputToHTML($pagename, $type, $args, &$opt) {
  ...
  if (isset($opt['list'])) {
    $list = MarkupRestore($opt['list']);
    $alist = [];
    if (strpos($list, "\n")!==false) {
      $alist = preg_split("/\n+/", $list, -1, PREG_SPLIT_NO_EMPTY);
    }
    elseif (strpos($list, ";")!==false) {
      $alist = preg_split("/; +/", $list, -1, PREG_SPLIT_NO_EMPTY);
    }
    if ($alist) {
      static $listcount = 0;
      $opt['list'] = 'pmdatalist'. (++$listcount);
      $alist = array_map('trim', $alist);
      $datalist = '';
      $datalist = "<datalist id='{$opt['list']}'>";
      foreach ($alist as $o) if ($o>'') {
        $datalist .= '<option value="'.PHSCQF($o).'"/>';
      }
      $datalist .= '</datalist>';
      $HTMLFooterFmt[$opt['list']] = $datalist;
    }
  }
  $attr = array();
...
function InputSelect($pagename, $type, $markup) {
  ...
  $attrlist = $InputTags['select'][':attr']?? array_diff($InputAttrs, array('value'));
  $attr = array();
  foreach($attrlist as $a) {
    if (!isset($opt[$a]) || $opt[$a]===false) continue;
    $attr[] = "$a='".str_replace("'", '&#39;', $opt[$a])."'";
...
$Conditions['e_preview'] = '(bool)$_REQUEST["preview"]';
...
SDVA($InputTags['e_form'], array(
  ':html' => "<form action='{\$PageUrl}?action=edit' method='post'
    ...
    />"));
SDVA($InputTags['e_textarea'], array( # not autofocus, breaks TextScrollTop
...
SDVA($InputTags['e_changesummary'], array(
  ...
  'placeholder' => XLHSC('Summary'),
  'value' => PHSC(stripmagic(@$_POST['csum']), ENT_QUOTES)));
SDVA($InputTags['e_minorcheckbox'], array( # configured in pmwiki.php:MergeLastMinorEdit
  ':html' => "<input type='checkbox' \$InputFormArgs />\$InputFormLabel",
  'name' => 'diffclass', 'value' => 'minor', 
  'label'=> XLHSC('This is a minor edit')));
...
if(IsEnabled($EnableNotSavedWarning, 1)) {
  # 1: confirm leave if this form has changes
  # 2: preview form, already has changes, confirm leave
  $InputTags['e_form']['data-confirmunload'] = @$_REQUEST['preview']? 2:1;
}
