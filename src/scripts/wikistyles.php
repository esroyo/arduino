...
if (IsEnabled($EnableStdWikiStyles,1)) {
  ...
  ## %newwin% opens links in a new window, %download% opens the Save-As dialog
  SDV($WikiStyle['newwin']['target'],'_blank');
  SDV($WikiStyle['download']['download'],'');
  ...
  SDVA($WikiStyleApply,array(
    ...
    'p' => 'p(?!\\s+class=)'));
  foreach(array('item', 'list', 'block', 'p', 'div', 'h') as $c)
  ...
  ## frames, floating frames, and floats moved to pub/lib/pmwiki-core.css
...
function ApplyStyles($x) {
  ...
  while ($parts) {
    ...
    foreach((array)$alist as $a=>$s) {
      $spanattr = $id = $title = ''; $stylev = array();
      ...
      // if ($title) $spanattr .= " {$ns}title='$title'";
      if ($spanattr) {
