...
function MarkupDirectives($m) {
  global $LinkWikiWords, $SpaceWikiWords, $HTMLPNewline;
  extract($GLOBALS["MarkupToHTML"]);
  $enabled = strtolower(strval(@$m[1]))!='no';
  
  switch ($markupid) {
    case 'linkwikiwords':  $LinkWikiWords  = $enabled; break;
    case 'spacewikiwords': $SpaceWikiWords = $enabled; break;
    case 'linebreaks':     $HTMLPNewline   = $enabled? '<br />' : ''; break;
    case 'messages': 
      $args = isset($m[1]) ? ParseArgs($m[1]) : array();
      return FmtMessages($pagename, $args);
  }
  return '';
}

function FmtMessages($pagename, $args) {
  global $MessagesFmt;
  $msg = array('default'=>'');
  foreach($MessagesFmt as $k=>$v) {
    $key = is_int($k)? 'default' : $k;
    if (isset($args['keys']) && !MatchNames($key, $args['keys'])) continue;
    if (!isset($msg[$key])) $msg[$key] = '';
    $msg[$key] .= MergeMessages($pagename, $v);
  }
  $out = '';
  $dclose = isset($args['details']) ? KeepBlock("</details>") : '';
  foreach($msg as $key=>$val) {
    if (!$val) continue;
    if (isset($args['details'])) {
      $open = strtolower($args['details']) == 'open'? " open='open'" : '';
      $summary = PHSC($key);
      $out .= KeepBlock("<details$open><summary>$summary</summary>")."\n";
    }
    $out .= "$val\n$dclose\n";
  }
  return PRR($out);
...
function MergeMessages($pagename, $msg) {
  $out = '';
  if (is_array($msg)) foreach($msg as $x) $out .= MergeMessages($pagename, $x);
  else {
    $msg = FmtPageName($msg, $pagename);
    if (strpos($msg, 'markup:')===0) $out .= substr($msg, 7);
    else $out .= KeepBlock($msg);
    $out .= "\n";
  }
  return $out;
}
...
function MarkupSetProperty($m){ # title, description, keywords
  ...
  switch ($markupid) {
    case 'title': 
      ...
      $title = SetProperty($pagename, 'title', $title, NULL, $EnablePageTitlePriority);
...
if (IsEnabled($EnableRelativeAttachLinks, 0))
  SDV($QualifyPatterns["/\\b(Attach:)([^\\s$UrlExcludeChars]+)/"],
    'cb_qualifyattach');

function cb_qualifyattach($m) { 
  extract($GLOBALS['tmp_qualify']);
  if (preg_match('!^(.+)/(.+)$!', $m[2], $n)) {
    $pagename = MakePageName($pagename, $n[1]);
    $m[2] = $n[2];
  }
  return "{$m[1]}$pagename/{$m[2]}";
}
...
## headings
Markup('^!', 'block', '/^(!{1,6})(?:#([a-z_][-a-z0-9_.]*)\\s)?(\\s*\\S.*)$/i', "MarkupHeadings");
function MarkupHeadings($m) {
  $len = strlen($m[1]);
  $id = $m[2]? " id='{$m[2]}'" : '';
  return "<:block,1><h$len$id>$m[3]</h$len>";
}
...
function MarkupDirectivesInit() {
  global $MarkupDirectiveFunctions;
...
if(isset($MarkupDirectiveFunctions)) {
  $PostConfig['MarkupDirectivesInit'] = 50;
}
